-- Fix ambiguous email variable in register_seller_phone
...
BEGIN
  ...
  v_email := clean_phone || '@sellers.maristdete.local';
  ...
  IF EXISTS (SELECT 1 FROM auth.users WHERE users.email = v_email) THEN RAISE EXCEPTION 'Phone already registered'; END IF;
  uid := gen_random_uuid();
  ...
  VALUES (instance, uid, 'authenticated', 'authenticated', v_email, crypt(pwd, gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}'::jsonb, jsonb_build_object('full_name', full_name_input, 'phone', phone_input), now(), now(), '', '', '', null);
  INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at)
  VALUES (uid::text, uid, jsonb_build_object('sub', uid::text, 'email', v_email), 'email', now(), now(), now());
  INSERT INTO public.marketplace_sellers (user_id, full_name, email, phone, seller_type, intended_items, agreement, status)
  VALUES (uid, full_name_input, v_email, phone_input, 'vendor', 'Seller Dashboard Registration', true, 'active')
  ...
  VALUES (seller_id, store_slug, store_name_input, 'Welcome to '||store_name_input, v_email, phone_input, 'active');
  RETURN json_build_object('user_id', uid, 'email', v_email, 'seller_id', seller_id, 'slug', store_slug);
END;
$$;
